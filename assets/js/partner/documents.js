(async function(){
  ...
  const files = {
    front: document.getElementById('doc-front'),
    back: document.getElementById('doc-back'),
    selfie: document.getElementById('doc-selfie')
  };
  ...
  document.getElementById('upload-docs').addEventListener('click', async ()=>{
    try{
      ...
      // Salva nello stato (imposta in_review se presenti file)
      await fetch('/.netlify/functions/partner-update-profile', {
        method:'POST',
        headers:{'Content-Type':'application/json','Authorization':`Bearer ${(await (await PartnerState.fetch()).id_token)||''}`},
        body: JSON.stringify({ docs: out, docs_status: 'in_review' })
      }).catch(()=>{});
