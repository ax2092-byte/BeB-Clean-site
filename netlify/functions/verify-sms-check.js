...
exports.handler = async (event) => {
  try{
    ...
    if (approved){
      const auth = event.headers.authorization||''; const idt = auth.startsWith('Bearer ')? auth.slice(7): null; const sub = decodeJwt(idt).sub;
      ...
      await fetch(`https://${domain}/api/v2/users/${encodeURIComponent(sub)}`, { method:'PATCH', headers:{'Authorization':`Bearer ${access_token}`,'Content-Type':'application/json'}, body: JSON.stringify(patch) });
    }
